...
class ScheduleServiceTest {
    ...
    @BeforeEach
    void setUp() {
        screen = new Screen("스크린1", "주소1", null);
    ...
    void createSchedule_success() {
        ...
        Schedule schedule = new Schedule(startAt, endAt, screen, movie);
        schedule.setIdForTest(100L);
    ...
    @Test
    @DisplayName("스케줄 생성 실패 - 중복 시간 존재")
    void createSchedule_fail_overlap() {
        ...
        Schedule overlappingSchedule = new Schedule(startAt.minusHours(1), endAt.plusHours(1), screen, movie);
        when(screenRepository.findById(screen.getId())).thenReturn(Optional.of(screen));
        when(movieRepository.findById(movie.getId())).thenReturn(Optional.of(movie));
        when(scheduleRepository.findOverlappingSchedules(screen.getId(), startAt, endAt))
